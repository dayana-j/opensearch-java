...
public class GrpcBulk {
        ...
        try {
            ...
            for (int i = 0; i < 100; i++) {
                ...
                operations.add(new BulkOperation.Builder().index(
                    new IndexOperation.Builder<SampleDoc>()
                        .index(indexName)
            ...
            LOGGER.info("Bulk response: took={}ms, errors={}, items={}",
                bulkResponse.took(), bulkResponse.errors(), bulkResponse.items().size());
            ...
            SearchResponse<SampleDoc> searchResponse = client.search(
                s -> s.index(indexName).size(5), SampleDoc.class
            );
            LOGGER.info("Search: found {} documents (showing first 5)",
    ...
    private static OpenSearchClient createClient(
        String hostname, int restPort, int grpcPort,
        String user, String pass, boolean https
    ) throws Exception {
        ...
        var grpcBuilder = GrpcTransport.builder(hostname, grpcPort)
