        ...
        try {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                ops.add(new BulkOperation.Builder().index(
                    new IndexOperation.Builder<GrpcBulk.SampleDoc>()
                        .index(indexName).id(String.valueOf(id))
            ...
            BulkResponse response = client.bulk(
                new BulkRequest.Builder().index(indexName).operations(ops).refresh(Refresh.True).build()
            );
