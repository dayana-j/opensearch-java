...
public class JwtAuthInterceptorTest {
    ...
    @Test public void testStaticToken() { assertEquals("my-token", new JwtAuthInterceptor(() -> "my-token").getTokenSupplier().get()); }

    @Test public void testSupplierCalledEachTime() {
    ...
    @Test(expected = IllegalArgumentException.class) public void testNullSupplierThrows() { new JwtAuthInterceptor(null); }

    @Test public void testBuilderWithJwt() {
    ...
    @Test public void testBuilderWithTlsAndJwt() {
        GrpcTransport t = GrpcTransport.builder("localhost", 9400)
            .jsonpMapper(new org.opensearch.client.json.jackson3.JacksonJsonpMapper())
            .tlsInsecure().jwtAuth(() -> "token").build();
        assertNotNull(t); try { t.close(); } catch (Exception e) { /* */ }
    }
}
