...
public class GrpcTransportTest {
    ...
    @Test public void testDefaultOptions() {
    ...
    @Test public void testCustomOptions() {
        GrpcTransportOptions o = GrpcTransportOptions.builder()
            .maxInboundMessageSize(50 * 1024 * 1024).keepAliveTime(30, TimeUnit.SECONDS)
            .keepAliveTimeout(10, TimeUnit.SECONDS).keepAliveWithoutCalls(true)
            .idleTimeout(5, TimeUnit.MINUTES).maxRetries(5).retryBackoff(200, TimeUnit.MILLISECONDS).build();
    ...
    @Test public void testUnsupportedEndpoint() {
        Endpoint<Object, Object, Object> fake = new Endpoint<Object, Object, Object>() {
            ...
            @Override public String requestUrl(Object r) { return "/_search"; }
    ...
    public void testBuilderRequiresMapper() { GrpcTransport.builder("localhost", 9400).build(); }

    @Test public void testBuilderCreatesTransport() {
        GrpcTransport t = GrpcTransport.builder("localhost", 9400).jsonpMapper(mapper).build();
        assertNotNull(t); assertNotNull(t.channel()); assertNotNull(t.grpcOptions());
        try { t.close(); } catch (Exception e) { /* ignore */ }
    }

    @Test public void testUnsupportedEndpointThrows() throws Exception {
        GrpcTransport t = GrpcTransport.builder("localhost", 9400).jsonpMapper(mapper).build();
        try {
            Endpoint<Object, Object, Object> fake = new Endpoint<Object, Object, Object>() {
                @Override public String method(Object r) { return "GET"; }
    ...
    static class MockRestTransport implements OpenSearchTransport {
        int callCount = 0; String lastUrl = null;
        @Override @SuppressWarnings("unchecked")
        public <R, S, E> S performRequest(R req, Endpoint<R, S, E> ep, @Nullable TransportOptions o) throws IOException {
            callCount++; lastUrl = ep.requestUrl(req);
            if (ep == BulkRequest._ENDPOINT) return (S) new BulkResponse.Builder().took(1).errors(false).items(Collections.emptyList()).build();
            return null;
        }
        @Override public <R, S, E> CompletableFuture<S> performRequestAsync(R r, Endpoint<R, S, E> e, @Nullable TransportOptions o) {
            return CompletableFuture.supplyAsync(() -> { try { return performRequest(r, e, o); } catch (IOException ex) { throw new RuntimeException(ex); } });
        }
    ...
    static class FailingGrpcTransport extends GrpcTransport {
        FailingGrpcTransport(JsonpMapper m) { super(null, m, GrpcTransportOptions.defaults(), null, null); }
        @Override public <R, S, E> S performRequest(R r, Endpoint<R, S, E> ep, @Nullable TransportOptions o) throws IOException {
            throw new TransportException("gRPC connection unavailable: test");
        }
        @Override public void close() {}
    }

    @Test public void testUnsupportedRoutedToRest() throws IOException {
    ...
    @Test public void testFallbackOnGrpcFailure() throws IOException {
        MockRestTransport rest = new MockRestTransport();
    ...
    @Test public void testNoFallbackWhenDisabled() throws IOException {
        MockRestTransport rest = new MockRestTransport();
        HybridTransport h = new HybridTransport(new FailingGrpcTransport(mapper), rest, false);
        BulkRequest req = new BulkRequest.Builder().operations(op -> op.delete(d -> d.id("1").index("t"))).build();
        try { h.performRequest(req, BulkRequest._ENDPOINT, null); fail(); }
        catch (TransportException e) { assertEquals(0, rest.callCount); }
    }
}
