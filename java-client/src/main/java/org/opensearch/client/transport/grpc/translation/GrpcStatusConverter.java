...
public final class GrpcStatusConverter {
     ...
     * @throws TransportException for connection/auth/transport-level errors
     * @throws OpenSearchException for server-side request errors with structured error responses
     * @throws UnsupportedOperationException for UNIMPLEMENTED status
     */
        ...
        switch (code) {
            ...
            case UNAVAILABLE:
                return new TransportException(
                    "gRPC connection unavailable: " + description, e
                );

            case DEADLINE_EXCEEDED:
                return new TransportException(
                    "gRPC request timed out: " + description, e
                );

            case UNAUTHENTICATED:
                return new TransportException(
                    "Authentication failed (401): " + description, e
                );

            case PERMISSION_DENIED:
                return new TransportException(
                    "Forbidden access (403): " + description, e
                );

            case RESOURCE_EXHAUSTED:
                return new TransportException(
                    "Rate limited (429): " + description, e
                );

            case INTERNAL:
                return new TransportException(
                    "Internal gRPC error (500): " + description, e
                );

            case DATA_LOSS:
                return new TransportException(
                    "gRPC data loss (500): " + description, e
                );

            case CANCELLED:
                return new TransportException(
                    "gRPC request cancelled: " + description, e
                );

            case UNKNOWN:
                return new TransportException(
                    "Unknown gRPC error: " + description, e
                );

            case UNIMPLEMENTED:
                return new UnsupportedOperationException(
                    "gRPC endpoint not implemented: " + description, e
                );
            ...
            default:
                return new TransportException(
                    "gRPC error [" + code.name() + "]: " + description, e
                );
        }
    }
